...
void PlayerObject::HandelInputAction(SDL_Event event, SDL_Renderer* screen)
{
	if (event.type == SDL_KEYDOWN) {
		...
		case SDLK_SPACE: {
			...
			p_bullet->set_bullet_type(BulletObject::LAZE);
...
void PlayerObject::CheckToMap(Map& map_data)
{
	...
	if (x1 >= 0 && x2 < MAP_MAX_X && y1 >= 0 && y2 < MAP_MAX_Y) {
		...
		if (x_val_ > 0) {
			...
			if (val1 == STATE_GEM || val2 == STATE_GEM || val1 == WIN_CUP || val2 == WIN_CUP) {
					map_data.tile[y1][x2] = 0;
					map_data.tile[y2][x2] = 0;
					IncreaseMoney();
					if (val1 == WIN_CUP || val2 == WIN_CUP) {
						SetWCD(true);
					}
				}
				else {
...
void PlayerObject::CenterMap(Map& map_data)
{
